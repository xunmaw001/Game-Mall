    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑游戏收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">游戏收藏管理</li>
                        <li class="breadcrumb-item active">编辑游戏收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">游戏收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 youxiCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="youxiCollectionTypesSelect" name="youxiCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var youxiCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var yonghuOptions = [];
    var youxiOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("youxiCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addyouxiCollection');
    ...
    // 查询列表
        <!-- 查询当前表的所有列表 -->
        function youxiCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youxi_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    youxiCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationYouxicollectiontypesSelect(){
            var youxiCollectionTypesSelect = document.getElementById('youxiCollectionTypesSelect');
            if(youxiCollectionTypesSelect != null && youxiCollectionTypesOptions != null  && youxiCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < youxiCollectionTypesOptions.length; i++) {
                    youxiCollectionTypesSelect.add(new Option(youxiCollectionTypesOptions[i].indexName,youxiCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(youxiCollectionTypesSelect != null && youxiCollectionTypesOptions != null  && youxiCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < youxiCollectionTypesOptions.length; i++) {
                if (youxiCollectionTypesOptions[i].codeIndex == ruleForm.youxiCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        youxiCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                youxiId: "required",
                yonghuId: "required",
                youxiCollectionTypes: "required",
            },
            messages: {
                youxiId: "游戏不能为空",
                yonghuId: "用户不能为空",
                youxiCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addyouxiCollection = window.sessionStorage.getItem("addyouxiCollection");
        if (addyouxiCollection != null && addyouxiCollection != "" && addyouxiCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("youxiCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationYouxicollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
