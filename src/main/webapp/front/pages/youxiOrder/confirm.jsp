    ...
    <!-- 标题 -->
            ...
            <div class="common_list_con clearfix">
                ...
                <ul class="goods_list_td clearfix"  v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="col02"><img :src="item.youxiPhoto"></li><!-- style="width: 100px;height: 100px;object-fit: cover;"-->
                    <li class="col03">{{item.youxiName}}</li>
			...
			<div class="common_list_con clearfix">
				<div class="settle_con" v-if="this.youxiOrderPaymentTypes == 1">
				...
				<div class="settle_con" v-if="this.youxiOrderPaymentTypes == 2">
		...
		<script>
			var vue = new Vue({
				...
				methods: {
					...
					,payClick() {
						let data={
                            youxis:localStorage.getItem('youxis'),
                            yonghuId: localStorage.getItem('userid'),
                            youxiOrderPaymentTypes: vue.youxiOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`youxiOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../youxiOrder/list.jsp';
							}
                       		 window.location.href='../youxiOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取游戏购买的清单列表
				var youxis = localStorage.getItem('youxis');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(youxis);
