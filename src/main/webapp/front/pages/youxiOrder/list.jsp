...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的游戏订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="youxiCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitYouxiCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部游戏订单</li>
                    <li v-for="item in youxiOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 60px;">{{item.youxiName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.youxiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 150px;">{{item.youxiOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.youxiOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.youxiOrderValue}}</td>
                            <td style="width: 80px;">{{item.youxiOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">
                                <%--<button v-if="item.youxiOrderTypes==1" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.youxiOrderTypes==3" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.youxiOrderTypes==1" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(youxiOrderTypes) {
                this.youxiOrderTypes = youxiOrderTypes;
                // 获取列表数据
                layui.http.request('youxiOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    youxiOrderTypes: (vue.youxiOrderTypes == 0)?null:vue.youxiOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('youxiOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    youxiOrderTypes: (this.youxiOrderTypes == 0)?null:this.youxiOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`youxiOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`youxiOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`youxiOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,submitYouxiCommentback(){
                var youxiCommentbackContent = $("#youxiCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("youxiOrder/commentback?id="+id+"&commentbackText="+youxiCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function youxiOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youxi_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function youxiOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youxi_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        http.request('youxiOrder/page', 'get',{
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('youxiOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            youxiOrderType: (vue.youxiOrderTypes == 0)?null:vue.youxiOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
