        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">游戏收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="youxiName" id="youxiName" placeholder="游戏名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="youxiUuidNumber" id="youxiUuidNumber" placeholder="游戏编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <div class="team-info">
                            <div class="team-row">
                                <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-6 col-xs-6 team-grids">
                                    <i @click="deleteYouxiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <div class="team-agile-img" @click="jumpCheck('../youxi/detail.jsp?id='+item.youxiId , item.youxiDelete == null?'':item.youxiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img :src="item.youxiPhoto?item.youxiPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                        <div class="view-caption">
                                            <div class="w3ls-info">
                                                <h4>{{item.youxiName}}</h4>
                                            </div>
                                            <b v-if="item.youxiNewMoney" style="float: right;display: inline-block;color: #FFFFFF;font-size: 17px;">
                                                {{item.youxiNewMoney}} RMB
                                            </b>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            youxiCollectionTypesList : [],
        // 级联表字典表数据
            shangxiaTypesList : [],
            youxiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteYouxiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('youxiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function youxiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youxi_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.youxiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#youxiName').val() != null && jquery('#youxiName').val() != "" && jquery('#youxiName').val() != "null" ) {
                param['youxiName'] = jquery('#youxiName').val();
            }
                     
            if (jquery('#youxiUuidNumber').val() != null && jquery('#youxiUuidNumber').val() != "" && jquery('#youxiUuidNumber').val() != "null" ) {
                param['youxiUuidNumber'] = jquery('#youxiUuidNumber').val();
            }
                                                                                                                                                                                                                                                                                                                                
            // 当前表
                                                
            //收藏只能查类型为1的
            param['youxiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('youxiCollection/page', 'get', param, function(res) {
